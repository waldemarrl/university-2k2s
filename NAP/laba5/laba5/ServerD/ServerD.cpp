...
bool GetRequestFromClient(char* name, short port, sockaddr* from, int* flen)////ожидания запроса клиентской программы
{
	char nameServer[50];
...
bool PutAnswerToClient(const char* name, sockaddr* to, int* lto) //подтверждения сервером  запроса  клиента на установку соединения
{
...
//	int optval = 1;
//
//	if (setsockopt(checkServer, SOL_SOCKET,		//уровень д-вия 
...
int main() {
	...
	try {
		...
		while (true) {
			...
			{
				std::cout << std::endl << PutAnswerToClient(name, (sockaddr*)&clnt, &lc); ////подтверждения сервером  запроса  клиента на установку соединения
			}
			else
			{
