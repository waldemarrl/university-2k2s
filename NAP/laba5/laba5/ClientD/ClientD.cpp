...
bool GetServerByName(const char* name, const char* call, sockaddr* from, int* flen) { //для поиска  сервера по его символическому имени и позывному
	hostent* host = gethostbyname(name);
	if (!host) throw SetErrorMsgText("gethostbyname:", WSAGetLastError());


	char* ip_addr = inet_ntoa(*(in_addr*)(host->h_addr)); //преобр-ние сет представления IPv4-адреса в символьный формат  
...
int main() {
	...
	try {
		...
		GetServerByName("DESKTOP-CNC5AQ2", call, (sockaddr*)&clnt, &lc);
